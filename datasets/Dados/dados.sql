...
-- # BUSCA SOLCITIAÇÃO E ATIVIDADE 


SELECT
    PWF.NUM_PROCES AS NumeroProcesso,
    HPS.NUM_SEQ_ESTADO AS EtapaAtividade,
    TAR.CD_MATRICULA AS ResponsavelAtividade,
    TAR.ASSIGN_START_DATE AS DataInicioAtividade,
    TAR.END_DATE AS DataFimAtividade,
    TAR.DSL_OBS_TAR AS Observacao,
    USR.LOGIN AS Login,
    USR.EMAIL As Email
FROM PROCES_WORKFLOW PWF
INNER JOIN HISTOR_PROCES HPS
    ON HPS.NUM_PROCES = PWF.NUM_PROCES
INNER JOIN TAR_PROCES TAR
    ON TAR.NUM_PROCES = HPS.NUM_PROCES AND TAR.NUM_SEQ_MOVTO = HPS.NUM_SEQ_MOVTO
INNER JOIN FDN_USERTENANT USR
        ON TAR.CD_MATRICULA = USR.USER_CODE
    where  TAR.END_DATE IS NULL;


    /**
      *@DADOS REDE SOCIAL FLUIG
      *
      */


 /* Busca Publicações */
 SELECT * FROM sociable WHERE sociableType = 'SocialPost' ORDER BY creationdate DESC

 /*Buscar Comentários*/
 SELECT * FROM sociable WHERE sociableType = 'SociableComment' ORDER BY creationdate DESC

 /*Buscar Curtidas do Usuários */
 SELECT * FROM sociablelike ORDER BY id DESC LIMIT 500

 /* Rede Social*/
 SELECT * FROM social ORDER BY creationdate DESC LIMIT 500
